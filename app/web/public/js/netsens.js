...
//loadJSON('jsons/devices.json',function(response) {
    devices = JSON.parse(response);
	console.log(devices);
    loadJSON('js/data/db/links.json',function(response) {
    //loadJSON('jsons/links.json',function(response) {
        links = JSON.parse(response);
	for (var i = 0; i<links.length; i++) {
    links[i].source = links[i].sourceDeviceId;
    links[i].target = links[i].targetDeviceId;
    delete links[i].sourceDeviceId;
    delete links[i].targetDeviceId;
}
	console.log(links);
...
var dragDrop = d3.drag().on('start', function (node) {
          ...
          var node = svg
            ...
            .append("circle")
			  ...
			  .attr("r", function(d) {return Math.max(2,Math.min(d.hits, (screen.height-280)/devices.length,20))})
   			  .call(dragDrop)
			  .style("stroke", "#25a9af")
          ...
          var simulation = d3.forceSimulation(devices)                 // Force algorithm is applied to data.nodes
              ...
              .force("Collide", d3.forceCollide(function(d) {return Math.max(2,Math.min(d.hits, (screen.height-280)/devices.length)) * 2 * 1.2}))
              .nodes(devices)
              .on("tick", ticked);	
